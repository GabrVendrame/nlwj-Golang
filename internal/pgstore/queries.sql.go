...
const createTripLinks = `-- name: CreateTripLinks :one
INSERT INTO links
...
type CreateTripLinksParams struct {
	TripID uuid.UUID
	Title  string
	Url    string
}

func (q *Queries) CreateTripLinks(ctx context.Context, arg CreateTripLinksParams) (uuid.UUID, error) {
	row := q.db.QueryRow(ctx, createTripLinks, arg.TripID, arg.Title, arg.Url)
	var id uuid.UUID
	err := row.Scan(&id)
	return id, err
}
...
SELECT id, trip_id, email, is_confirmed FROM participants WHERE trip_id = $1
`
...
SELECT id, destination, owner_email, owner_name, is_confirmed, starts_at, ends_at FROM trips WHERE id = $1
`
...
SELECT id, trip_id, title, url FROM links WHERE trip_id = $1
`
